...
// class PrimsMST{
		...
		// public List<Edge<Integer>> PrimsMST(Graph<Integer> graph){
			// BinaryMinHeap<Vertex<Integer>> minHeap=new BinaryMinHeap<>();
			// Map<Vertex<Integer>,Edge<Integer>> vertexToEdge=new HashMap<>();
			// List<Edge<Integer>> result=new ArrayList<>();
			// for(Vertex<Integer> v:graph.getyAllVertex()){
				// minHeap.add(Integer.MAX_VALUE,v);
			// }
			
			// Vertex<Integer> startvertex=graph.getyAllVertex().iterator().next();
			...
			// while(!minHeap.empty()){
				...
				// Vertex<Integer> current=minHeap.extractMin();
					...
					// for(Edge<Integer> edge:current.getEdges()){
						...
						// Vertex<Integer> adjcent=getVertexForEdge(current,edge);
						// if(minHeap.containsData(adjcent) &&minHeap.getWeight(adjcent)>edge.getWeight()){
							// minHeap.decrease(adjcent,edge.getWeight());
							// vertexToEdge.put(adjcent,edge);
						// }
					// }
			// }
			// return result;
	...
	// public static void main(String args[]){
			...
			// Collection<Edge<Integer>> edges=prims.primsMST(graph);
			// for(Edge<Integer> edge:edges){
				// System.out.prinltn(edge);		
			// }
 	// }
